<div>
    <x-modal.dialog wire:model.defer="show_modal">
        <x-slot name="title">
            {{ \App\Traits\Fmt::title('label.analyze') }}
        </x-slot>
        ...
        <x-slot name="footer">
            ...
            <div class="flex justify-center items-center">
                <x-button.danger type="button"
                                 wire:click="$emit('{{ \App\Http\Livewire\Dashboard\Analyze::CANCEL_SCHEDULE }}')">
                    <x-icon.cancel class="w-4 h-4 mr-1">
                        {{ \App\Traits\Fmt::text('label.btn.not-approve') }}
                    </x-icon.cancel>
                </x-button.danger>

                <x-button type="button" class="ml-1"
                          wire:click="$emit('{{ \App\Http\Livewire\Dashboard\Analyze::APPROVE_SCHEDULE }}')">
                    <x-icon.check class="w-4 h-4 mr-1">
                        {{ \App\Traits\Fmt::text('label.btn.yes-approve') }}
                    </x-icon.check>
                </x-button>
            </div>
